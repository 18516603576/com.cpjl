 		...
 		<div id="top">
					<div class="left">
					    <span class="big">${cName }&nbsp;</span> 
  						<a  id="thisPage" href="${pageContext.request.contextPath}/user/list?cName=${cName}"  class="on">用户列表</a>
  						<a href="javascript:new openWin({document:window.parent.document,width:'90%',height:'90%',title:'添加用户',src:'${pageContext.request.contextPath}/user/insert'}).start()" >添加用户</a>						
...
<table border="0" cellpadding="0" cellspacing="1"  id="mTable" >
   <tr class="captail">
		...
		<div class="right">
		     <form name="form2" id="form2" method="post" action="?cName=${cName }">
							 <input type="text" class="pt" name="wd" id="wd" value="${entity.wd}"  default_data="用户名" autocomplete="off">
	...
	<tr class="tr"> 
 		<td>
 		<c:if test="${list.id!=1 }">
 		<input type="checkbox" name="selId" class="pc" value="${list.id }">
 		</c:if>
 		</td>
 		<td>
 		<dl>
 		 <dt>
 		   <img style="width:102.4px; height: 76.8px" src="${ftpDomain }${list.avartar}" />
 		 <dt>
 		 <dd>
 		   <p class="title">${list.userName }</p>
 		   <p class="order"> ${list.realName } - ${list.position }</p>	    
 		 </dd>
 		 </dl>
        </td>
        
        <td>${list.isAble }</td>
 		<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm"   value="${list.lastLoginTime}" /></td>
 		<td>${list.loginTimes }</td>
		<td class="operate">
		<a href="javascript:new openWin({document:window.parent.document,width:'90%',height:'90%',title:'修改用户',src:'${pageContext.request.contextPath}/user/resetPassword?id=${list.id }'}).start()">重置密码</a>	 
		<a href="javascript:new openWin({document:window.parent.document,width:'90%',height:'90%',title:'修改用户',src:'${pageContext.request.contextPath}/user/update?id=${list.id }'}).start()">修改</a>
		 <c:if test="${list.id!=1 }">
  ...
  <script>
        ...
        $(document).delegate("#batchDel","click",function(){ 
			 init1.doo(true,$("input[name='selId']:checked"),'','${pageContext.request.contextPath}/user/deleteBatchDo','确定删除所选记录吗？',true);
		});
		$(document).delegate("a.del","click",function(){ 
 			 init1.doo(false,'',$(this),'${pageContext.request.contextPath}/user/deleteDo','确定删除当前记录吗？',true);
		});
