...
public class FTPClient extends FTP implements Configurable {
	 ...
	 * parserClass, which is passed to
	 ...
	 * the client's data port to initiate a data transfer. This is the default data
	 ...
	 * in passive mode, requiring the other server to connect to the first server's
	 ...
	 * connection mode set to <code> ACTIVE_LOCAL_DATA_CONNECTION_MODE </code>, the
	 * file type set to <code> FTP.ASCII_FILE_TYPE </code>, the file format set to
	...
	private boolean __storeFile(int command, String remote, InputStream local, HttpServletRequest request,
			String sessionName) throws IOException {
		return _storeFile(FTPCommand.getCommand(command), remote, local, request, sessionName);
	}

	/*
	 * 没有sessionName和request参数
	 * 
	 * @author jiangshanmn
	 */
	...
	protected boolean _storeFile(String command, String remote, InputStream local, HttpServletRequest request,
			String sessionName) throws IOException {
		Socket socket;
		...
		try {
			...
			//		false);
			this.copyStream(local, output, getBufferSize(), CopyStreamEvent.UNKNOWN_STREAM_SIZE, __mergeListeners(csl),
					false, request, sessionName);
	...
	protected long copyStream(InputStream source, OutputStream dest, int bufferSize, long streamSize,
			CopyStreamListener listener, boolean flush, HttpServletRequest request, String sessionName)
		...
		String sessionTransferredBytes = sessionName + "transferredBytes";
		session.setAttribute(sessionTransferredBytes, total);
	 ...
	 * 
	 * @param timeout
	 *            The default timeout in milliseconds that is used when opening a
	 ...
	 * <code>ACTIVE_LOCAL_DATA_CONNECTION_MODE</code>. No communication with the FTP
	 * server is conducted, but this causes all future data transfers to require the
	 * FTP server to connect to the client's data port. Additionally, to accommodate
	 * differences between socket implementations on different platforms, this
	 * method causes the client to issue a PORT command before every data transfer.
	 ***/
	...
	/***
	 * Set the current data connection mode to
	 * <code> PASSIVE_LOCAL_DATA_CONNECTION_MODE </code>. Use this method only for
	 * data transfers between the client and server. This method causes a PASV (or
	 * EPSV) command to be issued to the server before the opening of every data
	 * connection, telling the server to open a data port to which the client will
	 * connect to conduct data transfers. The FTPClient will stay in
	 * <code> PASSIVE_LOCAL_DATA_CONNECTION_MODE </code> until the mode is changed
	 * by calling some other method such as {@link #enterLocalActiveMode
	...
	/***
	 * Set the current data connection mode to
	 * <code> ACTIVE_REMOTE_DATA_CONNECTION </code>. Use this method only for server
	 * to server data transfers. This method issues a PORT command to the server,
	 * indicating the other server and port to which it should connect for data
	 * transfers. You must call this method before EVERY server to server transfer
	 * attempt. The FTPClient will NOT automatically continue to issue PORT
	 * commands. You also must remember to call {@link #enterLocalActiveMode
	 * enterLocalActiveMode() } if you wish to return to the normal data connection
	 * mode.
	 * <p>
	 * 
	 * @param host
	 *            The passive mode server accepting connections for data transfers.
	 * @param port
	...
	/***
	 * Set the current data connection mode to
	 * <code> PASSIVE_REMOTE_DATA_CONNECTION_MODE </code>. Use this method only for
	 * server to server data transfers. This method issues a PASV command to the
	 * server, telling it to open a data port to which the active server will
	 * connect to conduct data transfers. You must call this method before EVERY
	 * server to server transfer attempt. The FTPClient will NOT automatically
	 * continue to issue PASV commands. You also must remember to call
	 ...
	 * server when entering passive mode. If not in passive mode, returns null. This
	 * method only returns a valid value AFTER a data connection has been opened
	 * after a call to {@link #enterLocalPassiveMode enterLocalPassiveMode()}. This
	 * is because FTPClient sends a PASV command to the server only just before
	 ...
	 * a data connection, and not when you call {@link #enterLocalPassiveMode
	...
	/***
	 * Set the external IP address in active mode. Useful when there are multiple
	 * network cards.
	 * <p>
	 * 
	 * @param ipAddress
	 ...
	 * <code> FTP.ASCII_FILE_TYPE </code>, <code> FTP.BINARY_FILE_TYPE</code>, etc.
	 * The file type only needs to be set when you want to change the type. After
	 * changing it, the new type stays in effect until you change it again. The
	 * default file type is <code> FTP.ASCII_FILE_TYPE </code> if this method is
	 * never called.
	 * <p>
	 * <b>N.B.</b> currently calling any connect method will reset the mode to
	 * ACTIVE_LOCAL_DATA_CONNECTION_MODE.
	 ...
	 * of <code> FTP.ASCII_FILE_TYPE </code>, <code> FTP.BINARY_FILE_TYPE </code>,
	 ...
	 * The default file type is <code> FTP.ASCII_FILE_TYPE </code> if this method is
	 * never called. The format should be one of the FTP class
	 * <code> TEXT_FORMAT </code> constants, or if the type is
	 * <code> FTP.LOCAL_FILE_TYPE </code>, the format should be the byte size for
	 * that type. The default format is <code> FTP.NON_PRINT_TEXT_FORMAT </code> if
	 ...
	 *            The format of the file (one of the <code>_FORMAT</code> constants.
	 *            In the case of <code>LOCAL_FILE_TYPE</code>, the byte size.
	 *            <p>
	 * @return True if successfully completed, false if not.
	 * @exception FTPConnectionClosedException
	 *                If the FTP server prematurely closes the connection as a
	 ...
	 * given filename. The other server must have had a
	 ...
	 * unique filename based on the given filename. The other server must have had a
	 * <code> remoteRetrieve </code> issued to it by another FTPClient.
	 * <p>
	 * 
	 * @param filename
	 *            The name on which to base the filename of the file that is to be
	 *            stored.
	 * @return True if successfully completed, false if not.
	 * @exception FTPConnectionClosedException
	 *                If the FTP server prematurely closes the connection as a
	 ...
	 * Initiate a server to server file transfer. This method tells the server to
	 * which the client is connected to store a file on the other server using a
	 * unique filename. The other server must have had a
	 * <code> remoteRetrieve </code> issued to it by another FTPClient. Many FTP
	 * servers require that a base filename be given from which the unique filename
	 * can be derived. For those servers use the other version of
	 ...
	 * The other server must have had a <code> remoteRetrieve </code> issued to it
	 * by another FTPClient.
	 * <p>
	 * 
	 * @param filename
	 *            The name of the file to be appended to, or if the file does not
	 *            exist, the name to call the file being stored.
	 *            <p>
	 * @return True if successfully completed, false if not.
	 * @exception FTPConnectionClosedException
	 *                If the FTP server prematurely closes the connection as a
	 ...
	 * FTP commands to complete a transaction. These commands require some action by
	 * the programmer after the reception of a positive intermediate command. After
	 ...
	 * current file type is ASCII, line separators in the file are converted to the
	 ...
	 *                The CopyStreamException allows you to determine the number of
	 *                bytes transferred and the IOException causing the error. This
	...
	protected boolean _retrieveFile(String command, String remote, OutputStream local) throws IOException {
		...
		InputStream input = new BufferedInputStream(socket.getInputStream(), getBufferSize());
		if (__fileType == ASCII_FILE_TYPE) {
	 ...
	 * care of closing the parent data connection socket upon being closed. To
	 * finalize the file transfer you must call {@link #completePendingCommand
	 ...
	 *                server or receiving a reply from the server.
	 ***/
	public boolean storeFile(String remote, InputStream local, HttpServletRequest request, String sessionName)
			throws IOException {
		return __storeFile(FTPCommand.STOR, remote, local, request, sessionName);
	}
	 ...
	 * writing to it. The OutputStream itself will take care of closing the parent
	 * data connection socket upon being closed. To finalize the file transfer you
	 * must call {@link #completePendingCommand completePendingCommand } and check
	 * its return value to verify success.
	 * <p>
	 * 
	 * @param remote
	 *            The name to give the remote file.
	 ...
	 *            The name of the remote file.
	 * @param local
	 *            The local InputStream from which to read the data to be appended
	 *            to the remote file.
	 * @return True if successfully completed, false if not.
	 * @exception FTPConnectionClosedException
	 *                If the FTP server prematurely closes the connection as a
	 ...
	 * file are transparently converted to the NETASCII format (i.e., you should not
	 ...
	 * transfer you must call {@link #completePendingCommand completePendingCommand
	 ...
	 * file to the NETASCII format (i.e., you should not attempt to create a special
	 * OutputStream to do this). You must close the OutputStream when you finish
	 * writing to it. The OutputStream itself will take care of closing the parent
	 * data connection socket upon being closed. To finalize the file transfer you
	 * must call {@link #completePendingCommand completePendingCommand } and check
	...
	/**
	 * Query the server for supported features. The server may reply with a list of
	 * server-supported exensions. For example, a typical client-server interaction
	 * might be (from RFC 2389):
	 * 
	 * <pre>
	    C> feat
	 ...
	 *         if the feature is not found or the command failed. Check
	...
	/**
	 * Query the server for a supported feature. Caches the parsed response to avoid
	...
	/**
	 * Query the server for a supported feature with particular value, for example
	...
	/***
	 * Restart a <code>STREAM_TRANSFER_MODE</code> file transfer starting from the
	 ...
	 * 
	 * @param offset
	 *            The offset into the remote file at which to start the next file
	 *            transfer.
	 * @return True if successfully completed, false if not.
	 * @exception FTPConnectionClosedException
	 *                If the FTP server prematurely closes the connection as a
	 ...
	 * @exception IOException
	 *                If an I/O error occurs while either sending a command to the
	 *                server or receiving a reply from the server (and the default
	 ...
	 * error No files found.). If the directory is not empty, an array of filenames
	 ...
	 * @return The list of filenames contained in the given path. null if the list
	 *         could not be obtained. If there are no filenames in the directory, a
	 ...
	 * returned a 550 error No files found.). If the directory is not empty, an
	 * array of filenames in the directory is returned.
	 * <p>
	 * 
	 * @return The list of filenames contained in the current working directory.
	 *         null if the list could not be obtained. If there are no filenames in
	 ...
	 * returned array is determined by the<code> FTPFileEntryParser </code> used.
	 * <p>
	 * 
	 * @param pathname
	 *            The file or directory to list. Since the server may or may not
	 *            expand glob expressions, using them here is not recommended and
	 *            may well cause this method to fail.
	 *
	 ...
	 *                Thrown if the parserKey parameter cannot be resolved by the
	 *                selected parser factory. In the DefaultFTPEntryParserFactory,
	 *                this will happen when parserKey is neither the fully qualified
	 *                class name of a class implementing the interface
	 *                org.apache.commons.net.ftp.FTPFileEntryParser nor a string
	 *                containing one of the recognized keys mapping to such a parser
	 *                or if class loader security issues prevent its being loaded.
	 * @see org.apache.commons.net.ftp.parser.DefaultFTPFileEntryParserFactory
	 * @see org.apache.commons.net.ftp.parser.FTPFileEntryParserFactory
	 ...
	 *                server or receiving a reply from the server.
	 * @exception org.apache.commons.net.ftp.parser.ParserInitializationException
	 *                Thrown if the autodetect mechanism cannot resolve the type of
	 *                system we are connected with.
	 ...
	 *
	 * @param parserKey
	 *            A string representing a designated code or fully-qualified class
	 *            name of an <code> FTPFileEntryParser </code> that should be used
	 *            to parse each server file listing. May be {@code null}, in which
	 *            case the code checks first the system property
	 *            {@link #FTP_SYSTEM_TYPE}, and if that is not defined the SYST
	 *            command is used to provide the value. To allow for arbitrary
	 *            system types, the return from the SYST command is used to look up
	 *            an alias for the type in the {@link #SYSTEM_TYPE_PROPERTIES}
	 ...
	 * modification time of a file. The modification string should be in the ISO
	 * 3077 form "YYYYMMDDhhmmss(.xxx)?". The timestamp represented should also be
	 * in GMT, but not all FTP servers honour this.
	 *
	 * @param pathname
	 ...
	 * also be in GMT, but not all servers honour this.
	 *
	 ...
	 * @param timeval
	 *            The timestamp to set to, in <code>YYYYMMDDhhmmss</code> format.
	 * @return true if successfully set, false if not
	 * @throws IOException
	 *             if an I/O error occurs.
	 * @since 2.2
	 * @see <a href=
	 *      "http://tools.ietf.org/html/draft-somers-ftp-mfxx-04">http://tools.ietf.org/html/draft-somers-ftp-mfxx-04</a>
	 */
	 ...
	 * case of this class, configuring merely makes the config object available for
	 ...
	 * server. It depends on your ftp server if you need to call this method, also
	 * dont expect to get rid of hidden files if you call this method with "false".
	 *
	 ...
	 * network) and the client is coming from another internal network. In that case
	 * the data connection after PASV command would fail, while EPSV would make the
	 * client succeed by taking just the port.
	 *
	 * @param selected
	 *            value to set.
	 * @since 2.2
	...
	/**
	 * Set the listener to be used when performing store/retrieve operations. The
