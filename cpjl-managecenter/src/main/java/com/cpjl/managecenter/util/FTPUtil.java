...
public class FTPUtil {
	 ...
	 * @return
	 */
	public static boolean put(String filePath, InputStream is, FTPClientTemplate ftp) {
		String folderPath = filePath.substring(0, filePath.lastIndexOf("/"));
		ftp.mkdir(folderPath);
		boolean result = ftp.put(filePath, is, null);
		return result;
	}
	...
	public static boolean delete(String filePath) {
		FTPClientTemplate ftp = getFTPClientTemplate();
		boolean ret = ftp.delete(filePath);
		return ret;
	}

	/*
	 * 返回一个默认ftp客户端对象,在config.properties中
	 * 
	 */
	public static FTPClientTemplate getFTPClientTemplate() {
